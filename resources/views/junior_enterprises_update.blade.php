        ...
        <div class="min-w-screen min-h-screen bg-gray-100 flex items-center justify-center bg-gray-100 font-sans overflow-hidden">
            <div class="w-full lg:w-5/6">
                <div class="bg-white shadow rounded-lg p-6">
                    <form action="{{route('junior_enterprise.update', ['id' => request()->id ])}}" method="POST">
                        @csrf
                        <div class="grid lg:grid-cols-2 gap-6">
                            <div class="border focus-within:border-blue-500 focus-within:text-blue-500 transition-all duration-500 relative rounded p-1">
                                ...
                                <p>
                                    <input value="{{ $junior_enterprise->name }}" name="name" id="name" autocomplete="false" tabindex="0" type="text" class="py-1 px-1 text-gray-900 outline-none block h-full w-full" required>
                                </p>
                            </div>
                            <div class="border focus-within:border-blue-500 focus-within:text-blue-500 transition-all duration-500 relative rounded p-1">
                                <div class="-mt-4 absolute tracking-wider px-1 uppercase text-xs">
                                ...
                                <p>
                                    <select name="federation" id="federation" tabindex="0" class="py-1 px-1 outline-none block h-full w-full" required>
                                        @foreach($federations as $federation)
                                            @if ($junior_enterprise->federation_id == $federation->id)
